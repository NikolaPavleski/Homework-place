...
(9, 7),
(10, 6); 
SELECT genres.name FROM genres JOIN movie_genres ON genres.genre_id = movie_genres.genre_id JOIN movies ON movie_genres.movie_id = movies.movie_id WHERE movies.rating = 'R' GROUP BY genres.name HAVING COUNT(movies.movie_id) > 3;
SELECT directors.first_name, directors.last_name FROM directors JOIN movies ON directors.director_id = movies.director_id JOIN movie_revenues ON movies.movie_id = movie_revenues.movie_id GROUP BY directors.director_id HAVING SUM(movie_revenues.domestic_revenue + movie_revenues.international_revenue) > 500000000 AND COUNT(movies.movie_id) >= 2;
SELECT actors.first_name, actors.last_name FROM actors JOIN cast_members ON actors.actor_id = cast_members.actor_id JOIN movies ON cast_members.movie_id = movies.movie_id JOIN movie_genres ON movies.movie_id = movie_genres.movie_id JOIN genres ON movie_genres.genre_id = genres.genre_id JOIN actor_awards ON actors.actor_id = actor_awards.actor_id GROUP BY actors.actor_id HAVING COUNT(DISTINCT genres.genre_id) > 2 AND COUNT(actor_awards.award_id) > 0;
SELECT movies.title FROM movies JOIN reviews ON movies.movie_id = reviews.movie_id GROUP BY movies.movie_id HAVING COUNT(reviews.review_id) > 3 AND AVG(reviews.rating) > 7;
SELECT production_companies.name FROM production_companies JOIN movie_production_companies ON production_companies.company_id = movie_production_companies.company_id JOIN movies ON movie_production_companies.movie_id = movies.movie_id JOIN movie_revenues ON movies.movie_id = movie_revenues.movie_id WHERE movies.release_date > '2015-01-01' GROUP BY production_companies.name HAVING SUM(movie_revenues.domestic_revenue + movie_revenues.international_revenue) > 100000000;
SELECT movie_locations.country FROM movie_locations JOIN movies ON movie_locations.movie_id = movies.movie_id WHERE movies.budget > 150000000 GROUP BY movie_locations.country HAVING COUNT(movies.movie_id) > 2;
SELECT genres.name FROM genres JOIN movie_genres ON genres.genre_id = movie_genres.genre_id JOIN movies ON movie_genres.movie_id = movies.movie_id JOIN movie_awards ON movies.movie_id = movie_awards.movie_id JOIN awards ON movie_awards.award_id = awards.award_id WHERE awards.name = 'Oscar' GROUP BY genres.name HAVING AVG(movies.duration_minutes) > 120;
SELECT EXTRACT(YEAR FROM movies.release_date) AS release_year FROM movies GROUP BY EXTRACT(YEAR FROM movies.release_date) HAVING COUNT(movies.movie_id) > 3 AND AVG(movies.budget) > 50000000;
SELECT actors.first_name, actors.last_name FROM actors JOIN cast_members ON actors.actor_id = cast_members.actor_id JOIN movies ON cast_members.movie_id = movies.movie_id JOIN movie_revenues ON movies.movie_id = movie_revenues.movie_id WHERE cast_members.is_lead_role = true GROUP BY actors.actor_id HAVING COUNT(movies.movie_id) > 2 AND SUM(movie_revenues.domestic_revenue + movie_revenues.international_revenue) > 200000000;