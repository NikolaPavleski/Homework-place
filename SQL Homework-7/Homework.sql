...
BEGIN
    ...
    VALUES (new_movie_id, 188020017, 487100000);
END;
$$ LANGUAGE plpgsql;
...
SET domestic_revenue = 300000000, international_revenue = 850000000
WHERE movie_id = 1;
COMMIT;

--3
CREATE OR REPLACE FUNCTION update_created_at_movies() 
RETURNS TRIGGER AS $$
BEGIN
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trg_update_created_at_movies
BEFORE INSERT ON movies
FOR EACH ROW
EXECUTE FUNCTION update_created_at_movies();

--4
CREATE OR REPLACE FUNCTION prevent_future_release()
RETURNS TRIGGER AS $$
BEGIN
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trg_prevent_future_release
BEFORE INSERT ON movies
FOR EACH ROW
EXECUTE FUNCTION prevent_future_release();
...
BEGIN
    INSERT INTO movies (title, release_date, duration_minutes, rating, director_id, plot_summary, language, budget)
    VALUES (p_title, p_release_date, p_duration, p_rating, p_director_id, p_plot, p_language, p_budget)
    RETURNING movie_id INTO new_movie_id;

    INSERT INTO movie_genres (movie_id, genre_id)
    VALUES (new_movie_id, p_genre_id);
END;
$$;
CALL add_movie_with_genre(
    'Tenet', '2020-08-26', 150, 'PG-13', 1,
    'A secret agent embarks on a time-bending mission to prevent World War III.', 'English', 200000000, 1
);