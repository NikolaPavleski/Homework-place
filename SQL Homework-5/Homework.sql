...
BEGIN
    RETURN QUERY
    SELECT m.movie_id, m.title, m.release_date, m.rating
    FROM movies m
...
CREATE OR REPLACE FUNCTION get_director_filmography(p_director_id INT)
RETURNS TABLE (
    movie_id INT,
    title VARCHAR,
    release_date DATE,
...
BEGIN
    RETURN QUERY
    SELECT m.movie_id, m.title, m.release_date, m.duration_minutes, m.rating
    FROM movies m
...
CREATE OR REPLACE FUNCTION calculate_actor_age(birthdate DATE)
RETURNS INT
AS $$
BEGIN
    RETURN DATE_PART('year', AGE(CURRENT_DATE, birthdate))::INT;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION has_won(p_actor_id INT)
RETURNS BOOLEAN
AS $$
BEGIN
    RETURN EXISTS (
...
END;
$$ LANGUAGE plpgsql;
SELECT first_name, last_name, has_won(actor_id) AS has_awards
FROM actors;